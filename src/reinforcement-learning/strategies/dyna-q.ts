...
interface DynaQOpts extends LearningOpts {
  // number of updates to the estimates that will run from the
  // learned model
  nPlanning: number
}

export const dynaQ = (env: Environment, opts?: DynaQOpts) => {
  const {
    gamma = 1.0,
    initAlpha = 0.5,
    minAlpha = 0.01,
  ...
  _.times(nEpisodes, (episode) => {
    ...
    while (true) {
      ...
      for (let i = 0; i < nPlanning; ++i) {
        const sumQ = nArrayReduce(Q, (sum: number, val: number) => sum + val, 0)
        if (sumQ === 0) break

        // simulate an experience by the learned model
        const state: any = _.sample(Array.from(visitedStates))
        const action: any = _.sample(Array.from(takenActionsByState[state]))
