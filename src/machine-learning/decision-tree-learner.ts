...
export class DecisionTreeLearner {
  ...
  constructor(examples: Examples, attributes: DiscreteRandomVariable<any>[]) {
    this.examples = examples
    this.attributes = attributes
  }

  learn() {
    const learnDecisionTree = (
      examples: Examples,
      attributes: DiscreteRandomVariable<any>[],
    ...
    this.tree = learnDecisionTree(this.examples, this.attributes)
  }

  // predict(example: any) {
  //   let walker = this.tree
  //   while (walker.children.length > 0) {
  //     const idx = _.findIndex(this.attributes, { name: walker.attribute.name })
  //     const next = _.find(walker.children, { value: example[idx] })
  //     if (next) {
  //       walker = next
  //     }
  //   }
